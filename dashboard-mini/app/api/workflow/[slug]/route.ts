...
export async function GET(
  ...
  try {
    ...
    console.log(`🔍 Repository path: ${repo.repoPath}`);
    ...
    // Fetch workflows from GitHub API with pagination (per_page=100)
    let allWorkflows: GitHubWorkflow[] = [];
    let page = 1;
    let hasMorePages = true;

    while (hasMorePages) {
      const url = `https://api.github.com/repos/${owner}/${repoName}/actions/workflows?per_page=100&page=${page}`;
      console.log(`🔍 Fetching page ${page} from GitHub: ${url}`);
      ...
      console.log(`📄 Page ${page}: Got ${pageWorkflows.length} workflows (total so far: ${allWorkflows.length + pageWorkflows.length})`);
      allWorkflows = allWorkflows.concat(pageWorkflows);

      // If less than 100 returned, we've reached the last page
      hasMorePages = pageWorkflows.length === 100;
      page++;

      // Safety guard to prevent accidental infinite loops
      if (page > 50) {
        console.warn('Workflow pagination exceeded 50 pages, stopping early.');
        hasMorePages = false;
      }
    }
