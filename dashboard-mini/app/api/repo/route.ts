...
export async function GET() {
  try {
    ...
    // For dashboard-mini, we only expect one repository
    if (allRepos.length > 1) {
      console.warn('Dashboard-mini detected multiple repositories, returning only the first one');
      allRepos.splice(1); // Keep only the first repository
    }
