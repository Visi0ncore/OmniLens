...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if we already have a repository (dashboard-mini constraint)
    const { loadUserAddedRepos } = await import('@/lib/db-storage');
    const existingRepos = await loadUserAddedRepos();
    if (existingRepos.length > 0) {
      return NextResponse.json({ 
        error: 'Dashboard-mini only supports one repository. Please remove the existing repository first.',
        message: 'Dashboard-mini is designed to monitor a single repository.'
      }, { status: 409 });
    }
