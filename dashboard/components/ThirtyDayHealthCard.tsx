...
export default function ThirtyDayHealthCard({ repoSlug, repoPath }: Props) {
  ...
  const storageKey = React.useMemo(() => {
    ...
    return `report-cache-30-${id}-${endForKey}`;
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Compute health deltas by comparing consecutive days
      ...
      for (let i = 0; i < sortedDays.length; i++) {
        ...
        // pass/fail counts and avg runtime
        let totalSec = 0, c = 0;
        for (const r of runs) {
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-lg">30 Day Health</CardTitle>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          <div className="h-20" />
